const YOUR_PORTFOLIO = 'Tvoje Portfolio';
...
angular.module('MainModule', []).controller('MainController',['$scope', 'StockService', '$timeout', function($scope, StockService, $timeout) {
  ...
  function updateEditButtonText() {
    $scope.editButtonText = $scope.isEditing ? 'Hotovo' : 'Editovať';
  }
      ...
      var success = function(result) {
        ...
        $scope.$apply(() => {
          ...
          if ($scope.myLineChart) {
            ...
            updateVisualizations(newLineChartData);
  ...
  function addSentiment(stock) {
    ...
    if (stock.recentSentiment === 'Positive') {
      stock.recentSentiment = 'Pozitívne'
    } else if (stock.recentSentiment === 'Negative') {
      stock.recentSentiment = 'Negatívne'
    }
  }
  ...
  function updateVisualizations(newLineChartData) {
    //update Line Chart
    if (newLineChartData) {
      $timeout(function() {
